...
export default function POSPage() {
    ...
    fetch("/api/orders")
      ...
      .then((d) => {
        const active = (d.orders || []).filter(
          (o: ActiveOrder) => o.status === "PENDING" || o.status === "CONFIRMED"
        );
        setActiveOrders(active);
      });
  }

  async function cancelOrderFromList(orderId: string) {
    setCancelLoading(orderId);
    const res = await fetch(`/api/orders/${orderId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "cancel", reason: cancelReason || "Dibatalkan dari POS" }),
    });
    if (res.ok) {
  ...
  async function placeOrder() {
    ...
    try {
      ...
      // Jika meja (order bertahap) ΓÇö tampilkan notifikasi saja
      if (tableId) {
        setReceipt({
    ...
    <>
      ...
      {!activeShift ? (
        <div className="bg-amber-50 border-b border-amber-100 px-4 py-2 flex items-center justify-between shrink-0">
          <span className="text-xs text-amber-700 font-medium">Belum ada shift aktif ΓÇö buka shift sebelum mulai transaksi</span>
          <button
      ...
      {activeOrders.length > 0 && (
        <div className="bg-blue-50 border-b border-blue-100 shrink-0">
          ...
          >
            ...
            <span className="text-xs text-blue-500">{showOrders ? "Tutup" : "Lihat"}</span>
          </button>
          {showOrders && (
              ...
              {activeOrders.map((order) => (
                <div key={order.id} className="bg-white rounded-xl border border-blue-100 p-3">
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            {cancelItemConfirm === item.id ? (
                              <div className="flex items-center gap-1">
                                ...
                                <button onClick={() => setCancelItemConfirm(null)} className="text-xs text-gray-400 hover:text-gray-600 px-1">✕</button>
                              </div>
                            ) : (
                              <button onClick={() => setCancelItemConfirm(item.id)} className="text-xs text-red-400 hover:text-red-600" title="Batalkan item">✕</button>
                            )}
                          </div>
                        </div>
                      ))}
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-bold text-emerald-600">{formatRupiah(order.totalAmount)}</span>
                    <div className="flex items-center gap-2">
                      {expandedOrder === order.id && <span className="text-xs text-gray-400">{order.items.length} item</span>}
                      {cancelConfirm === order.id ? (
                        <div className="flex items-center gap-2">
                          <input type="text" value={cancelReason} onChange={(e) => setCancelReason(e.target.value)} placeholder="Alasan (opsional)" className="text-xs border border-gray-200 rounded-lg px-2 py-1 w-32 focus:outline-none focus:ring-1 focus:ring-red-400" autoFocus />
                          <button onClick={() => cancelOrderFromList(order.id)} disabled={cancelLoading === order.id} className="text-xs font-bold text-white bg-red-500 hover:bg-red-600 px-2 py-1 rounded-lg transition-colors disabled:opacity-50">
                            {cancelLoading === order.id ? "..." : "Semua"}
                          </button>
                          <button onClick={() => { setCancelConfirm(null); setCancelReason(""); }} className="text-xs text-gray-400 hover:text-gray-600 px-1">Batal</button>
                        </div>
                      ) : (
                        <button onClick={() => setCancelConfirm(order.id)} className="text-xs text-red-500 hover:text-red-700 font-medium">✕ Batal</button>
                      )}
                    </div>
                  </div>
                </div>
      ...
      >
        <span className="text-xl">≡ƒ¢Æ</span>
        {items.length > 0 && (
        ...
        {view === "menu" && (
          ...
          <div className="p-4 border-b border-gray-100">
            ...
            {tableInfo ? (
              ...
              >
                <option value="">≡ƒÑí Takeaway</option>
                {tables.map((t) => (
                  <option key={t.id} value={t.id}>
                    ≡ƒ¬æ Meja {t.number} ({t.capacity} orang) {t.status === "OCCUPIED" ? "ΓÜá∩╕Å Terisi" : ""}
                  </option>
                ))}
              </select>
            )}
          ...
          <div className="flex-1 overflow-y-auto p-4">
              ...
              <div className="mb-4">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-xs font-semibold text-amber-700 uppercase tracking-wide">
                    ≡ƒôï Order Aktif ({activeOrder.orderNumber})
                  </h3>
                    ...
                    <div key={item.id} className="flex items-center justify-between bg-amber-50 rounded-lg px-3 py-2">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[10px] text-gray-500">
                          {formatRupiah(item.unitPrice)} ├ù {item.quantity} = {formatRupiah(item.subtotal)}
                        </div>
                      </div>
                      <button
            ...
            {items.length === 0 ? (
              <div className="text-center py-12">
                <div className="text-4xl mb-3">≡ƒ¢Æ</div>
          ...
          <div className="p-4 border-t border-gray-100 space-y-3">
            ...
            {tableId ? (
              <div className="space-y-2">
                ...
                >
                  {loading ? "Menyimpan..." : `≡ƒô¥ Simpan ke Meja (${formatRupiah(total())})`}
                </button>
                <button
                  onClick={payAllTable}
                  disabled={loading}
                ...
                >
                  {loading ? "Memproses..." : `≡ƒÆ│ Bayar Semua`}
                </button>
              </div>
            ) : (
              <button
        ...
        {view === "tables" && (
          <div className="flex-1 p-4 overflow-y-auto">
            ...
            <div className="flex gap-2 mb-4">
              ...
              >
                ≡ƒôà Reservasi {reservations.filter((r) => r.status === "PENDING" || r.status === "CONFIRMED").length > 0 && (
                  ...
                  <div className="text-center">
                    ...
                    {t.status === "OCCUPIED" && (
                      <div className="mt-2 space-y-0.5">
                        ...
                        {t.firstOrderAt && (
                          <div className="text-[10px] text-amber-600">
                            ΓÅ▒∩╕Å {tableDuration(t.firstOrderAt)}
                          </div>
                        )}
                      </div>
                    )}
                ...
                {reservations.length === 0 ? (
                  ...
                  reservations.map((r) => (
                    <div key={r.id} className="bg-white border border-gray-100 rounded-xl p-4 space-y-2">
                      ...
                      <div className="text-xs text-gray-500">
                        ≡ƒòÉ {new Date(r.reserveAt).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })}
                        {r.table && ` ┬╖ ≡ƒ¬æ Meja ${r.table.number}`}
                        {` ┬╖ ${r.duration} menit`}
                      </div>
                      ...
                      <div className="flex gap-2 pt-1">
                        {r.status === "PENDING" && (
                          <>
                            <button onClick={() => reservationAction(r.id, "confirm")} className="text-[10px] px-2 py-1 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100">Γ£à Konfirmasi</button>
                            <button onClick={() => reservationAction(r.id, "cancel")} className="text-[10px] px-2 py-1 bg-red-50 text-red-600 rounded-lg hover:bg-red-100">Γ£ò Batal</button>
                          </>
                        )}
                        {r.status === "CONFIRMED" && (
                          <>
                            <button onClick={() => reservationAction(r.id, "checkin")} className="text-[10px] px-2 py-1 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100">≡ƒ¬æ Check-in</button>
                            <button onClick={() => reservationAction(r.id, "cancel")} className="text-[10px] px-2 py-1 bg-red-50 text-red-600 rounded-lg hover:bg-red-100">Γ£ò Batal</button>
                          </>
                        )}
                        {r.status === "PENDING" && (
                          <button onClick={() => reservationAction(r.id, "noShow")} className="text-[10px] px-2 py-1 bg-gray-100 text-gray-500 rounded-lg hover:bg-gray-200">≡ƒÜ½ Tidak Datang</button>
                        )}
        ...
        <div className="fixed inset-0 bg-black/50 flex items-end lg:items-center justify-center z-50 p-0 lg:p-4">
          <div className="bg-white rounded-t-2xl lg:rounded-2xl w-full max-w-sm shadow-xl max-h-[80vh] flex flex-col">
            <div className="p-5 border-b border-gray-100 flex items-center justify-between">
              ...
              <button onClick={() => setShowTableDetail(null)} className="text-gray-400 hover:text-gray-600">Γ£ò</button>
            </div>
            <div className="flex-1 overflow-y-auto p-5 space-y-3">
              {showTableDetail.status === "OCCUPIED" && showTableDetail.activeOrderCount > 0 ? (
                <>
                  ...
                  >
                    ≡ƒÆ│ Bayar Semua ({formatRupiah(showTableDetail.activeOrderTotal)})
                  </button>
                </>
              ) : (
                <button
        ...
        <div className="fixed inset-0 bg-black/50 flex items-end lg:items-center justify-center z-50 p-0 lg:p-4">
          <div className="bg-white rounded-t-2xl lg:rounded-2xl w-full max-w-md shadow-xl max-h-[85vh] flex flex-col">
            <div className="p-5 border-b border-gray-100 flex items-center justify-between">
              ...
              <button onClick={() => setShowReserveModal(false)} className="text-gray-400 hover:text-gray-600">Γ£ò</button>
            </div>
            ...
            <div className="p-5 border-t border-gray-100 flex gap-2">
              ...
              <button onClick={createReservation} className="flex-1 py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-bold hover:bg-emerald-700 transition-colors">≡ƒôà Simpan Reservasi</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 flex items-end lg:items-center justify-center z-50 p-0 lg:p-4">
          <div className="bg-white rounded-t-2xl lg:rounded-2xl w-full max-w-md shadow-xl max-h-[85vh] flex flex-col">
            <div className="p-5 border-b border-gray-100 flex items-center justify-between">
              ...
              <button onClick={() => { setShowManageTables(false); refreshTables(); }} className="text-gray-400 hover:text-gray-600">Γ£ò</button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-2">
              ...
              {items.length === 0 ? (
                <div className="text-center py-8">
                  <div className="text-3xl mb-2">≡ƒ¢Æ</div>
      ...
      {receipt && (
        <div className="fixed inset-0 bg-black/60 flex items-end lg:items-center justify-center z-50 p-0 lg:p-4">
          <div className="bg-white rounded-t-2xl lg:rounded-2xl shadow-2xl w-full max-w-xs overflow-hidden max-h-[90vh] flex flex-col">
            ...
            {/* Receipt content ΓÇö this area is printed */}
